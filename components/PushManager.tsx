...
export default function PushManager() {
  ...
  useEffect(() => {
    if ('serviceWorker' in navigator && 'PushManager' in window) {
      ...
      // Only prompt if they haven't explicitly denied it yet
  ...
  return (
    <div className="fixed bottom-24 left-4 right-4 md:left-auto md:right-4 md:w-80 bg-white p-4 rounded-2xl shadow-xl border border-slate-100 z-[100] animate-in slide-in-from-bottom-10">
      <button onClick={() => setShowPrompt(false)} className="absolute top-2 right-2 p-1 text-slate-400 hover:text-slate-600"><X size={16}/></button>
      <div className="flex gap-3">
          ...
          <div>
              ...
              <button onClick={subscribe} className="mt-3 bg-slate-900 text-white px-4 py-2 rounded-xl text-xs font-bold active:scale-95 transition-transform w-full">Allow Notifications</button>
          </div>
      </div>
    </div>
  )
