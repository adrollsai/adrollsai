...
async function run() {
    console.log("=== STARTING TS DIRECT GENERATION TEST ===");
    const { callGemini, createKieImageTask } = await import('../utils/external-apis');
...
`;
    ...
    // 2. Parallel image generation using Kie.ai
...
PROMPT: ${p}`,
        ...
        while (pendingTasks.some(t => t.status === 'pending') && (Date.now() - startTime) < timeoutMs) {
            ...
            const pollPromises = pendingTasks.map(async (task) => {
                ...
                try {
                    ...
                    if (res.ok) {
                        const checkData = await res.json() as any;
                        ...
                        if (status === 'succeeded' || status === 'completed' || status === 'success') {
                            ...
                            let imageUrl = resultData?.image_url || resultData?.imageUrl || resultData?.url || resultData?.output_url || resultData?.outputUrl;
                            if (!imageUrl && (checkData.data?.resultJson || checkData.resultJson)) {
                                try {
                                    const parsed = JSON.parse(checkData.data?.resultJson || checkData.resultJson);
                                    const parsedUrls = parsed.resultUrls || parsed.result_urls || [parsed.url];
                                    imageUrl = Array.isArray(parsedUrls) ? parsedUrls[0] : parsedUrls;
                                } catch (e) {}
                            }
                            if (imageUrl) {
                    ...
                    console.error(`[Test] Error polling task ${task.id}:`, err.message);
                }
            });
        ...
        console.error("[Test] Kie Image generation failed:", err.message);
    }
