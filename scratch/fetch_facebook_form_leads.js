...
async function run() {
    ...
    const formsUrl = `https://graph.facebook.com/v19.0/${pageId}/leadgen_forms?fields=id,name,status,created_time&access_token=${token}`;
    ...
    forms.forEach(f => {
        console.log(`- Form ID: ${f.id} | Name: ${f.name} | Status: ${f.status} | Created: ${f.created_time}`);
    });
