...
export default function AccountsPage() {
  ...
  const handleToggleSubscription = async (accountId: string, currentStatus: string) => {
    const isCurrentlyActive = ['active', 'trialing', 'pro'].includes(currentStatus?.toLowerCase() || '')
    const newStatus = isCurrentlyActive ? 'suspended' : 'active'
    
    // Optimistically update UI
    setAccounts(prev => prev.map(acc => acc.id === accountId ? { ...acc, subscription_status: newStatus } : acc))
    ...
    if (updateError) {
      console.error("[ACCOUNTS] Toggle subscription error:", updateError)
      // Rollback on error
      setAccounts(prev => prev.map(acc => acc.id === accountId ? { ...acc, subscription_status: currentStatus } : acc))
    }
  }
                    ...
                    {currentUser?.role === 'super_admin' && (
                      <div className="flex items-center gap-2 shrink-0 mr-2 border-r border-slate-200 pr-3">
                        <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">
                          {['active', 'trialing', 'pro'].includes(root.subscription_status?.toLowerCase() || '') ? 'Active' : 'Stopped'}
                        </span>
                        <button
                          onClick={() => handleToggleSubscription(root.id, root.subscription_status)}
                          className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none ${
                            ['active', 'trialing', 'pro'].includes(root.subscription_status?.toLowerCase() || '') ? 'bg-blue-600' : 'bg-slate-200'
                          }`}
                        >
                          <span
                            className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
                              ['active', 'trialing', 'pro'].includes(root.subscription_status?.toLowerCase() || '') ? 'translate-x-6' : 'translate-x-1'
                            }`}
                          />
                        </button>
                      </div>
                  ...
                  {root.children.map((child: any) => (
                    <div key={child.id} className="bg-white/50 border border-slate-200 rounded-xl p-4 flex items-center justify-between gap-3 hover:bg-white transition-all group">
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        {currentUser?.role === 'super_admin' && (
                          <div className="flex items-center gap-2 shrink-0 mr-1.5 border-r border-slate-100 pr-2.5">
                            <span className="text-[9px] font-bold text-slate-400 uppercase tracking-wider">
                              {['active', 'trialing', 'pro'].includes(child.subscription_status?.toLowerCase() || '') ? 'Active' : 'Stopped'}
                            </span>
                            <button
                              onClick={() => handleToggleSubscription(child.id, child.subscription_status)}
                              className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors focus:outline-none ${
                                ['active', 'trialing', 'pro'].includes(child.subscription_status?.toLowerCase() || '') ? 'bg-blue-600' : 'bg-slate-200'
                              }`}
                            >
                              <span
                                className={`inline-block h-3.5 w-3.5 transform rounded-full bg-white transition-transform`}
                                style={{
                                  transform: ['active', 'trialing', 'pro'].includes(child.subscription_status?.toLowerCase() || '') ? 'translateX(18px)' : 'translateX(2px)'
                                }}
                              />
                            </button>
                          </div>
