...
export default function LeadProfilePage() {
    ...
    const effectiveRecordingUrl = useMemo(() => {
        ...
        if (leadHistory && leadHistory.length > 0) {
            const item = leadHistory.find((h: any) => h.description && h.description.startsWith('🎙️ CALL_JSON:'))
            if (item) {
                try {
                    const parsed = JSON.parse(item.description.replace('🎙️ CALL_JSON:', '').trim())
                    if (parsed.recording_url) return parsed.recording_url
                } catch (e) {}
            }
        }
        return null
    }, [lead?.voice_recording_url, leadHistory])

    const effectiveTranscript = useMemo(() => {
        if (lead?.voice_call_transcript) {
            if (Array.isArray(lead.voice_call_transcript) && lead.voice_call_transcript.length > 0) {
                return lead.voice_call_transcript
            }
            if (typeof lead.voice_call_transcript === 'string') {
                try {
                    const parsed = JSON.parse(lead.voice_call_transcript)
        ...
        return []
    }, [lead?.voice_call_transcript, leadHistory])

    const effectiveSummary = useMemo(() => {
        if (lead?.voice_call_summary) return lead.voice_call_summary
        if (leadHistory && leadHistory.length > 0) {
            const item = leadHistory.find((h: any) => h.description && h.description.startsWith('🎙️ CALL_JSON:'))
            if (item) {
                try {
        ...
        return null
    }, [lead?.voice_call_summary, leadHistory])
                            ...
                            <div className="p-4.5 pt-2 border-t border-slate-100 space-y-4 animate-in fade-in duration-200">
                                ...
                                {effectiveSummary && (
                                    <div className="bg-slate-50/70 p-4 rounded-xl border border-slate-200/70">
                                        <span className="block text-[9px] font-black text-indigo-700 uppercase tracking-wider mb-1 flex items-center gap-1">
                                            ✨ AI Call Summary
                                        </span>
                                ...
                                {effectiveRecordingUrl ? (
                                    <div className="bg-linear-to-r from-purple-50/80 via-indigo-50/60 to-blue-50/50 p-4 rounded-2xl border border-indigo-100 shadow-xs space-y-2.5">
                                        <div className="flex items-center justify-between">
                                            <div className="flex items-center gap-2">
                                                <span className="p-1 bg-indigo-600 text-white rounded-lg text-xs">🎙️</span>
                                                <span className="text-[11px] font-black text-indigo-950 uppercase tracking-wider">
                                                    Call Audio Recording
                                                </span>
                                                <span className="text-[9px] font-black bg-indigo-200/80 text-indigo-800 px-2 py-0.5 rounded-full uppercase">
                                                    MP3
                                                </span>
                                            </div>
                                            <a 
                                                href={effectiveRecordingUrl.includes('media.vobiz.ai') ? `/api/voice/recording/stream?url=${encodeURIComponent(effectiveRecordingUrl)}&leadId=${lead.id}` : effectiveRecordingUrl} 
                                                target="_blank" 
                                                rel="noreferrer" 
                                                download={`call_${lead.name || lead.phone || 'recording'}.mp3`}
                                                className="text-[11px] font-bold text-indigo-600 hover:text-indigo-800 transition-colors flex items-center gap-1 hover:underline cursor-pointer"
                                            >
                                                Download MP3
                                            </a>
                                        </div>
                                ...
                                ) : (lead.voice_call_status === 'completed' || effectiveTranscript.length > 0) ? (
                                    <div className="bg-slate-50 border border-slate-200/80 p-3 rounded-xl flex items-center justify-between text-xs">
                                        <span className="text-slate-500 font-medium flex items-center gap-2">
                                            ...
                                            Audio recording being finalized by carrier (will appear here shortly)
                                        </span>
                                    </div>
                                ) : null}
                                ...
                                {effectiveTranscript.length > 0 && (
                                    <div className="pt-3 border-t border-slate-100 space-y-3">
                                        <div className="flex items-center justify-between">
                                            <span className="text-xs font-black text-slate-800 uppercase tracking-wider flex items-center gap-1.5">
                                                ...
                                                <span className="text-[10px] font-extrabold text-indigo-700 bg-indigo-50 border border-indigo-100 px-2 py-0.5 rounded-full">
                                                    {cleanTranscript(effectiveTranscript).length} messages
                                                </span>
                                            </span>
                                            <button 
                                                type="button"
                                        ...
                                        {showTranscript && (
                                            <div className="p-4 bg-slate-50 border border-slate-200/70 rounded-2xl max-h-80 overflow-y-auto space-y-3.5 custom-scrollbar shadow-inner">
                                                {cleanTranscript(effectiveTranscript).map((msg: any, index: number) => (
                                                    <div key={index} className={`flex flex-col ${msg.role === 'agent' ? 'items-start' : 'items-end'}`}>
                                                        <span className="text-[9px] font-black uppercase tracking-wider text-slate-400 mb-1 flex items-center gap-1">
                                                            {msg.role === 'agent' ? '🎙️ AI Voice Agent' : `👤 ${lead.name || 'Lead'}`}
                                                        </span>
                                                        <span className={`text-xs p-3.5 rounded-2xl max-w-[85%] font-medium leading-relaxed shadow-xs ${
                                                            msg.role === 'agent' 
