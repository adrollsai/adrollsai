          ...
          <div className="bg-white rounded-[2rem] p-8 shadow-sm border border-slate-200/60 max-w-2xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-300">
            <div className="flex items-center justify-between pb-4 border-b border-slate-100">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-xl font-bold text-slate-900">Android Call & Recording Settings</h2>
                  <p className="text-xs text-slate-500 font-medium">Configure local phone call logs & audio recording folder path sync.</p>
                </div>
              </div>
              <button 
            ...
            <div className="space-y-6">
              ...
              <div className="flex items-center justify-between p-5 bg-slate-50 rounded-2xl border border-slate-100">
                <div>
                  <h4 className="font-bold text-sm text-slate-800">Auto-Sync Call Logs & Recordings</h4>
                  <p className="text-xs text-slate-500 mt-1 max-w-xs font-medium">Automatically track call durations, connected status & audio files.</p>
                </div>
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full bg-slate-50 border border-slate-200 py-3.5 px-4 rounded-2xl text-xs font-bold outline-none text-slate-800 focus:border-blue-500 transition-all cursor-pointer"
                >
              ...
              <div className="pt-4 border-t border-slate-100 space-y-3">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold text-slate-800 font-black uppercase tracking-wider">
                    Attach Audio Recording to Lead Call
                  </span>
                  <span className="text-[10px] text-slate-400 font-semibold font-medium">Upload phone recording file</span>
                </div>
                <input
                  type="file"
                  ref={recordingFileInputRef}
                  accept="audio/*,.m4a,.mp3,.wav,.amr,.aac"
                  onChange={(e) => {
                ...
                <div className="flex gap-2">
                  <input
                    type="tel"
                    value={recordingPhone}
                    onChange={(e) => setRecordingPhone(e.target.value)}
                    placeholder="Lead Phone Number (e.g. 8288835235)"
                    className="flex-1 bg-slate-50 border border-slate-200 py-3 px-3.5 rounded-2xl text-xs font-bold outline-none text-slate-800 focus:border-blue-500 transition-all"
                  />
                  <button
                    type="button"
                    onClick={() => recordingFileInputRef.current?.click()}
                    className="bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold px-3 py-2 rounded-2xl text-xs transition-all flex items-center gap-1.5 cursor-pointer"
                  >
                    <Upload size={14} />
                    <span className="truncate max-w-[100px]">{recordingFile ? recordingFile.name : 'Pick Audio File'}</span>
                  </button>
                </div>
                <button
                  type="button"
                  onClick={async () => {
                    if (!recordingFile) {
                      toast.error('Please pick an audio recording file first')
                      return
                    }
                    ...
                    try {
                      ...
                      if (recordingPhone.trim()) {
                        formData.append('phoneNumber', recordingPhone.trim())
                      }
                      ...
                      if (res.ok && data.success) {
                        toast.success('Call recording uploaded & linked to lead timeline!')
                        setRecordingFile(null)
                        setRecordingPhone('')
                      } else {
                        toast.error(data.error || 'Failed to upload call recording')
                      }
                    } catch (e: any) {
                  ...
                  className="w-full bg-blue-600 hover:bg-blue-700 text-white font-extrabold py-3 px-4 rounded-2xl text-xs shadow-sm transition-all active:scale-98 disabled:opacity-50 cursor-pointer flex items-center justify-center gap-2"
                >
                  <Mic size={14} />
                  <span>{isUploadingRecording ? 'Uploading & Linking Recording...' : 'Upload & Attach Recording to Lead'}</span>
                </button>
              </div>
            </div>
          </div>
