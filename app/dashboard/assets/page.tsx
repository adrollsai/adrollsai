...
export default function AssetsPage() {
  ...
  // 4. Handle WhatsApp Share (Click -> Download UI -> Share Sheet)
  const handleShareWhatsApp = async () => {
    ...
    try {
      ...
      if (selectedAsset.type === 'video') {
         // Because videos are too large to share as a blob reliably, we use a direct app-link protocol
         // This opens the WhatsApp app DIRECTLY without going through a Safari tab
      ...
      // 1. Show the user it is downloading
      setShareProgressText('Downloading image...');
      const response = await fetch(selectedAsset.url);
      const blob = await response.blob();
      
      // 2. Prepare for share
      setShareProgressText('Opening share menu...');
      
      // Dynamically get the exact extension so iOS Safari doesn't reject it
      ...
      // 3. Trigger Native Share Sheet (NO browser links)
      if (navigator.canShare && navigator.canShare({ files: [file] })) {
          shareData.files = [file];
          await navigator.share(shareData);
      } else {
          // If the device/browser fundamentally doesn't support file sharing
          throw new Error("File sharing not supported natively.");
      }

    } catch (error: any) {
      // If the user manually cancels the iOS Share Sheet, it throws an AbortError. We safely ignore that.
      if (error.name === 'AbortError') return;
      
      console.error("Share failed:", error);
      alert("Direct image sharing isn't supported on your current browser. Please tap 'Download High-Res' to save it to your camera roll, then share it from there.");
    } finally {
          ...
          <div className="bg-white w-full max-w-sm rounded-[2rem] shadow-2xl animate-in zoom-in-95 duration-200 max-h-[90vh] flex flex-col overflow-hidden">
            ...
            <div className="p-5 overflow-y-auto">
              ...
              <div className="flex flex-col gap-3">
                ...
                {/* WhatsApp Button WITH DOWNLOADING UI */}
                <button 
