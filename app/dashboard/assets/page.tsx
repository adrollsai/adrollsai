...
type Asset = {
  ...
  url: string
  property_id?: string
  caption?: string // <-- Added so typescript knows the db has this
}
...
export default function AssetsPage() {
  ...
  // 4. Handle WhatsApp Share
  const handleShareWhatsApp = () => {
    if (!selectedAsset) return
    const whatsappText = encodeURIComponent(`${caption ? caption + '\n\n' : ''}${selectedAsset.url}`)
  ...
  const handleUniversalPost = async () => {
    ...
    // Targets explicitly set to only FB & IG now
          ...
          {filteredAssets.map((asset) => (
            <div 
                ...
                onClick={() => { 
                    ...
                    // AUTO-POPULATE THE CAPTION FROM DB HERE
