...
export async function POST(req: Request) {
    try {
        ...
        try {
            // Replace: const records = await dns.resolveTxt(domain);
            // With this logic to check the verification subdomain:

            const verifySubdomain = `adrolls-verify.${domain.replace('app.', '')}`;
            const records = await dns.resolveTxt(verifySubdomain);
