...
`;
        ...
        if (isEnglish) {
            ...
            languageInstruction = "The script dialogue MUST be written in Hinglish, mixing native Hindi Devanagari script (Hindi characters) and standard English/Roman letters. Specifically, all words that do NOT exist in the English language (e.g. Hindi words like 'kya', 'hai', 'aur', 'ke liye', 'apna', 'dekhie', 'miliye', 'sath', 'khushi') MUST be written in Hindi (using Devanagari characters, e.g., 'क्या', 'है', 'और', 'के लिए', 'अपना', 'देखिए', 'मिलिए', 'साथ', 'खुशी'). All words that DO exist in the English language (e.g. 'dream home', 'perfect space', 'luxury flat', 'living room', 'security', 'location', 'get in touch', 'modern modular kitchen') MUST be written in English using Roman characters. Do NOT transliterate English dictionary words into Devanagari script. Keep them in standard Roman characters. For example: 'Mohali में अपना dream home ढूंढ रहे हो? तो देखिए ये luxury 3 BHK flat. Premium features और best security के साथ get in touch करें!'";
        }
...
0.2. CRITICAL FIRST-LINE TARGET AUDIENCE CALLOUT & VISUAL HOOK RULE:
   - The very first line of the spoken dialogue (Scene 1, first 2 seconds) MUST call out the target audience explicitly (e.g. if selling homes in Mohali, call out home buyers in Mohali in the first line, like: "मोहाली में अपना dream home ढूंढ रहे हो?").
   - ${isEnglish ? 'The script\'s spoken dialogue MUST be entirely in English from the very first word. No Hindi, Hinglish, or Devanagari script.' : 'The script\'s spoken dialogue MUST be written in Hinglish. To guarantee flawless pronunciation by the voice model, you MUST write proper nouns, location names (e.g., write "न्यू चंडीगढ़" instead of "New Chandigarh", "मोहाली" instead of "Mohali", "जीरकपुर" instead of "Zirakpur"), units (e.g., write "कनाल" instead of "Kanal", "बी-एच-के" instead of "BHK"), and Hindi words in native Devanagari script. Only keep standard, simple English dictionary words (like "dream home", "perfect space", "luxury flat", "living room", "security", "get in touch") in standard Roman characters. Do NOT transliterate these simple English words to Devanagari.'}
