    ...
    try {
                    ...
                    try {
                        ...
                        if (activeMembers.length > 0 && groupCampaigns.length > 0) {
                            ...
                            if (matchesCamp) {
                                ...
                                console.log(`[99acres Webhook] Group distribution assigned lead to ${selectedMember.name} (${selectedMember.userId}) for rule ${aut.title}`);
                                break;
                            }
                        }
                    } catch (pErr) {
                        console.error('[99acres Webhook] Error evaluating group rule:', pErr);
                    }
                }
            }
        } catch (distErr) {
        ...
        if (!assignedAgentId) {
            try {
                    ...
                    for (const aut of fallbackGroupAutomations) {
                        try {
                            ...
                            if (activeMembers.length > 0) {
                                ...
                                console.log(`[99acres Webhook] Fallback group distribution assigned lead to ${selectedMember.name} (${selectedMember.userId}) for rule ${aut.title}`);
                                break;
                            }
                        } catch (pErr) {
                            console.error('[99acres Webhook] Error evaluating fallback group rule:', pErr);
                        }
                    }
                }
                ...
                console.error('[99acres Webhook] Error evaluating fallback distribution:', fbDistErr);
            }
        }
