...
export const maxDuration = 300 // Max 5 minutes for processing single user
...
export async function POST(request: Request) {
  try {
    ...
    console.log(`[EOD Report Worker] Processing EOD report for profile ID: ${profileId}...`)
    ...
    if (profile.enable_eod_report === false || !profile.email) {
      return NextResponse.json({ message: 'EOD report is disabled or email is missing' })
    }

    const startOfToday = new Date(Date.now() - 24 * 60 * 60 * 1000)

    // B. Query CRM Leads data
    ...
    // C. Query today's Voice Calls activity
    ...
    if (callsErr) {
      console.error(`[EOD Report Worker] Error querying call history for ${profile.id}:`, callsErr)
    }

    const callsTodayCount = callHistory?.length || 0

    // D. Query today's WhatsApp Messages activity
    ...
    if (waErr) {
      console.error(`[EOD Report Worker] Error querying WhatsApp messages for ${profile.id}:`, waErr)
    }
    ...
    // E. Analyze Meta Campaigns (if connected)
