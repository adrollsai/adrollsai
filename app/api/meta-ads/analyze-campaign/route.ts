...
export async function POST(request: Request) {
    ...
    try {
        ...
        const fields = 'id,name,status,daily_budget,lifetime_budget,budget_remaining,insights{spend,impressions,clicks,actions,ctr,cpc,cpm},adsets{id,name,status,daily_budget,lifetime_budget,insights{spend,impressions,clicks,actions},optimization_goal,billing_event,targeting},ads{id,name,status,creative{id,name,object_story_spec},insights{spend,impressions,clicks,actions}}';
        const fbUrl = `${FB_GRAPH_URL}/${campaignId}?fields=${fields}&date_preset=maximum&access_token=${creds.token}`;
