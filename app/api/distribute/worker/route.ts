...
// Vercel/Next.js max duration setting
...
export async function POST(request: Request) {
  ...
  for (const item of items) {
    try {
      ...
      // E. Increment Counter (FIXED: No .catch needed, just ignore return)
  ...
  if (pendingCount && pendingCount > 0) {
      console.log(`[Worker] Triggering next chunk. Remaining: ${pendingCount}`)
