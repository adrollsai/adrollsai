...
async function runTemplateBroadcast() {
  ...
  (leadsRes.data || []).forEach(l => {
    ...
    recipientMap.set(formatted, { name: firstName, phone: formatted, lead_id: l.id });
  });
  ...
  (chatsRes.data || []).forEach(c => {
    ...
    if (!recipientMap.has(formatted)) {
      ...
      recipientMap.set(formatted, { name: firstName, phone: formatted, chat_id: c.id });
    }
  });

  const recipients = Array.from(recipientMap.values());
  console.log(`\n🚀 Broadcasting APPROVED META TEMPLATE (nobogent_offer_promo_v1) with IMAGE HEADER & INTERESTED BUTTON to ${recipients.length} LEADS...\n`);
  ...
  for (let i = 0; i < recipients.length; i++) {
    ...
    try {
      ...
      if (data.messages && data.messages[0]?.id) {
        successCount++;
        console.log(`✅ [${i+1}/${recipients.length}] Delivered Approved Template + Image + Button to +${r.phone} (${greetingName}) - ID: ${data.messages[0].id}`);

        // Update database with media_type 'image'
        ...
        console.error(`❌ [${i+1}/${recipients.length}] Failed for +${r.phone}:`, data.error?.message || JSON.stringify(data));
        failCount++;
      }
    } catch (err) {
      console.error(`❌ Exception for +${r.phone}:`, err.message);
      failCount++;
    }
  ...
  console.log(`\n===================================================`);
  console.log(`🎉 APPROVED META TEMPLATE BROADCAST COMPLETE:`);
