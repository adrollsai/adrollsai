...
async function main() {
    ...
    for (let i = 0; i < clipTaskIds.length; i++) {
        ...
        console.log(`Querying Clip Task ${i + 1}: ${tid}...`);
        const res = await fetch(`https://api.kie.ai/api/v1/jobs/recordInfo?taskId=${tid}`, {
            headers: { 'Authorization': `Bearer ${kieApiKey}` }
        });
        const json = await res.json();
        console.log(`Clip ${tid} State:`, json.data?.state);
        const url = extractUrlFromKie(json);
        if (!url) throw new Error(`Could not extract URL for clip task ${tid}! JSON: ${JSON.stringify(json)}`);
        console.log(`Clip ${i + 1} URL:`, url);
        clipUrls.push(url);
    }

    console.log(`Querying Audio Task: ${audioTaskId}...`);
    const audioRes = await fetch(`https://api.kie.ai/api/v1/jobs/recordInfo?taskId=${audioTaskId}`, {
        headers: { 'Authorization': `Bearer ${kieApiKey}` }
    });
    const audioJson = await audioRes.json();
    console.log(`Audio ${audioTaskId} State:`, audioJson.data?.state);
    const audioUrl = extractUrlFromKie(audioJson);
    if (!audioUrl) throw new Error(`Could not extract URL for audio task ${audioTaskId}! JSON: ${JSON.stringify(audioJson)}`);
    console.log("Audio URL:", audioUrl);

    console.log("\n=== STEP 2: Downloading Media Files ===");
    const tempDir = path.join(os.tmpdir(), `khushi_grok_${Date.now()}`);
    ...
    console.log("=== STEP 3: Concatenating Clips with FFmpeg ===");
    ...
    // Scale background music volume from video track to 10% (0.1) and mix with 100% (1.0) voiceover track
    ...
    try {
        ...
        console.warn("Complex mix fallback to standard merge...", e.message);
    ...
    const r2Key = `generated/${userId}/grok_khushi_${Date.now()}.mp4`;
    ...
    const finalR2Url = `${R2_PUBLIC_URL}/generated/${userId}/grok_khushi_${Date.now()}.mp4`;
